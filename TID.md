...
기능적으로는 무엇을 먼저할 지 모호해 정보처리기사를 준비하며 배웠던 SDLC 바탕으로 하는 게 도움이 되겠다 싶어 간트차트를 크게 `요구사항-설계-구현-테스트-유지보수` 단계로 나누었다.
...
협업을 하면서 아무것도 정해지지 않은 상황에서 의견을 내보라고 하면 아무것도 나오지 않는 걸 알게 됐다. 뭐라도 만들어두고 거기서 수정을 하는 식으로 해야 의견이 나올 것 같았고 단시간 내에 대략적인 결과물을 만들어내기 위해 부트스트랩을 사용한 레이아웃을 채택하게 되었다.
