...
계속 바뀌는 테이블 때문에 Getter, Setter, Tostring 메서드 호출하는 게 귀찮아서 찾아봤는데 `lombok`이라고 @Data 같이 어노테이션하면 자동으로 실행해준다는 라이브러리를 찾아서 적용해서 팀원들에게 알려주려 했으나 오류가 날 수 있다는 주의점을 보고 어떤 상황에 에러가 날 지 몰라 안전성 이슈로 그만두었다. 일일이 입력하는 게 은근히 귀찮고 `MyBatis` 로 데이터를 받을 수 있기 때문에 `DTO를 왜 쓰지 않고 Map으로 하는 지` 알 것 같았다.  
